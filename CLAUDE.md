...
- `clubs.join_policy` (`'approval' | 'open'`, 기본 `approval`): 가입 방식. 클럽 상세에서 "승인제 가입 / 누구나 가입" 라벨로 노출. ※ 현재 `joinClub` 액션은 정책과 무관하게 항상 `pending`으로 insert한다(open 즉시 가입 미구현 — TODO).
...
- `/clubs/[id]` — 클럽 상세 + 가입 신청(`joinClub`, pending). 회원 명단(로스터)은 **정회원(`is_full_member`)에게만** 조회·표시(게스트·비회원은 RLS로 빈 결과).
