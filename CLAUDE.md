...
- **PWA + Web Push (VAPID)** — 앱 스토어 비용 없이 알림 제공. iOS는 홈 화면 추가 시에만 푸시가 동작한다.
...
- ⚠️ **이미 적용된 마이그레이션 파일은 절대 in-place 수정하지 말 것**: `db push`는 파일명(버전) 기준으로 적용 여부를 추적하므로, 이미 push된 파일의 내용을 고쳐도 **재실행하지 않는다**(조용히 무시). 스키마를 바꾸려면 **항상 새 마이그레이션 파일**을 추가한다. (실제로 `*_match_media.sql`을 `video_url`로 적용한 뒤 같은 파일을 `videos jsonb`로 고쳤다가, DB엔 반영 안 돼 상세 페이지가 새 컬럼을 조회하다 `data=null`→404 났다. 해소: `*_match_videos_jsonb.sql`로 분리.)
...
- **인덱스**: 핫 쿼리 경로는 이미 커버돼 있다 — 마이그레이션의 복합 인덱스 13개(`matches(club_id,match_date desc)`, `posts(club_id,created_at desc)`, `comments(post_id)`, `club_members(club_id,status)`·`(user_id)`, `notifications(user_id,read_at)`, `payments(club_id,status)` 등) + **PK/unique가 나머지를 커버**(`match_attendances`=`unique(match_id,user_id)`, `match_results`=PK `match_id`, `match_team_defs`·`match_team_scores`=PK `(match_id,team)`, `comment_mentions`=PK `(comment_id,user_id)` → 매치 상세의 하위테이블 `.eq/.in(match_id)` 조회가 인덱스 탐). ⚠️ **쓰기가 잦은 테이블(`match_attendances`=투표마다, `notifications`=이벤트마다)에 저빈도 읽기용 인덱스를 추가하지 말 것** — 쓰기 비용이 이득을 넘는다(예: 회원 프로필의 user_id-only 참석조회, 알림 정렬은 현 규모에선 인덱스 불필요). 회원 수백+로 커지면 그때 재검토.
...
Next.js 16은 `middleware.ts` 대신 **`proxy.ts`** 규칙을 쓴다(함수명도 `proxy`). 보호 경로 리다이렉트 로직은 `updateSession` 안에 있다(공개 경로: `/login`, `/auth/*`, `/api/push/*`).

⚠️ **PWA 파일은 `proxy.ts`의 matcher에서 반드시 제외한다** (`manifest.webmanifest`·`sw.js`). manifest 요청은 기본적으로 **자격증명 없이(쿠키 미포함)** 나가므로, matcher에 걸리면 **로그인 상태여도** 미들웨어가 `/login`으로 307을 내고 브라우저는 manifest를 못 읽는다 → `display: standalone`이 적용되지 않는다. 실제로 이것 때문에 **iOS 홈 화면 앱이 standalone으로 안 뜨고 스플래시가 통째로 안 나왔다**(link·PNG는 전부 정상 서빙 중이었는데도). standalone이 아니면 `apple-touch-startup-image`는 조건상 표시되지 않는다. `sw.js`도 같은 이유로 공개해야 한다(막히면 서비스워커 등록 실패 → 푸시 수신 불가). matcher의 확장자 예외(`svg|png|jpg…`)는 이 둘을 커버하지 못하니 **파일명을 직접 적어야 한다**. 새 공개 정적 파일(`robots.txt`·`sitemap.xml` 등)을 추가할 때도 같은 점검이 필요하다 — 증상은 "로그아웃 상태에서 `curl -o /dev/null -w '%{http_code}' <URL>`이 307".
...
**서버 컴포넌트 쿼리는 병렬로**: 페이지 로딩 시간 = 서버가 도는 모든 조회의 합이고, 무료 티어·콜드 스타트 지연이 순차 `await`마다 곱해진다. 서로 **독립인** Supabase 조회는 `Promise.all`로 묶는다. 의존 쿼리(A 결과로 B를 필터)만 순차 유지하고, **가드(`notFound`/`redirect`)는 배치를 다 받은 뒤 원래 순서대로** 검사한다(병렬화해도 최종 404/리다이렉트 결과는 동일). 조건부 쿼리는 권한 없으면 `null` 브랜치로 배열에 태워 스킵한다(예: 클럽 상세의 로스터=정회원만·가입대기=회장/총무만 → `isFullMember ? supabase... : null`, 결과는 `res?.data ?? []`). **적용 현황: 배치할 게 있는 서버 컴포넌트 페이지 전부(13개)에 반영됨** — 목록/상세 9개(홈 6→3, 클럽 상세 4→2, 매치 목록/상세, 커뮤니티 목록/상세, 회비, 알림, 회원 프로필 6→2, 게시글 상세 5→2) + 폼 4개(매치·커뮤니티 각 new/edit). 나머지 서버 페이지 2개(`/me`=getUser→profile 의존 쿼리, `/clubs/new`=getUser뿐)는 묶을 독립 쿼리가 없어 대상이 아니다(서버 페이지 총 15개 + 클라이언트 `/login`). 무거웠던 회원 프로필·게시글 상세가 대표적. **신규 서버 컴포넌트 페이지도 이 패턴을 따르고, 함께 `loading.tsx`(스켈레톤)를 둘 것**(아래 "디자인/UI 컨벤션 > 로딩 스켈레톤" 참고).
...
- `/auth/signout` — POST, 로그아웃(현재 기기, `scope` 기본값). 세션은 도메인별 쿠키라 로컬·프로덕션 로그인은 서로 독립(한쪽 로그인이 다른 쪽을 풀지 않음), Supabase 기본은 다중 세션 허용. "다른 기기 모두 로그아웃"은 `/me` 보안 섹션 참고.
...
- `/me` — 내 프로필 수정 (이름·연락처·**아바타 업로드**) + 알림(푸시) 토글 + **보안**. 폼은 `app/me/profile-form.tsx`(RHF+Zod, `lib/schemas/profile.ts`), 액션 `updateProfile`(`app/me/actions.ts`). 푸시 토글은 `components/push/push-toggle.tsx`. 아바타는 Storage `avatars` 버킷에 `{uid}/…` 경로로 업로드(공개 읽기 + 본인 폴더에만 쓰기, `*_avatars_storage.sql`). **보안 섹션**: "다른 기기 모두 로그아웃"(`sign-out-others.tsx` → 액션 `signOutOtherDevices`, `supabase.auth.signOut({ scope: "others" })`) — 현재 기기 세션은 유지하고 나머지 세션의 리프레시 토큰만 폐기(기기 분실·공용 PC 대비). 다중 세션을 강제 단일화하는 대신 사용자가 직접 정리하는 방식(전체 로그아웃은 `/auth/signout`). 확인 다이얼로그는 `ModalPortal`.
...
- **로딩 스켈레톤 / 페이지 전환 (성능)**: 페이지 이동이 "클릭해도 반응 없다가 뒤늦게 확 바뀌는" 느낌은 App Router가 **대상 서버 컴포넌트의 데이터 페칭이 끝나야 화면을 교체**하기 때문 — Suspense 경계(`loading.tsx`)가 없으면 이전 페이지에 그대로 머문다. 그래서 주요 라우트마다 `loading.tsx`를 두어 클릭 즉시 스켈레톤으로 전환한다(홈·클럽 상세·매치 목록/상세·커뮤니티 목록/상세·회비·알림·내프로필·회원프로필). 스켈레톤 조각은 공용 `components/ui/skeleton.tsx` — `Skeleton`(라임 틴트 블록 + `footmate-shimmer` 스윕), `SkeletonCard`(글래스 카드 레시피), `SkeletonFrame`(배경 오브+그리드 프레임, `width` prop으로 페이지별 폭 맞춤), `SkeletonHeader`(상단 바). ⚠️ `loading.tsx`는 페이지 대신 렌더되므로(배경을 layout이 아니라 **페이지가 소유**) **배경 프레임까지 스스로 그려야** 프레임이 안 깜빡인다 → `SkeletonFrame`으로 감싼다. 서버 쿼리가 무거운 신규 라우트엔 `loading.tsx`를 함께 추가할 것.
- **브랜드 에셋(로고/아이콘/OG)**: 로고 마크 = 라임 그라디언트(`#84cc16`→`#4d7c0f`) **2단 바운스 궤적 + 도트**(축구공 리터럴 없음), 배지는 **순백(`#ffffff`) 라운드 타일**. 아이콘 세트: `public/app-icon-{128,192,512}` + `app-icon-maskable-512`(안전영역 78% 축소) + `apple-touch-icon.png`(180) + `app/favicon.ico`(16·32·48). iOS 스플래시 세트는 `public/splash/*.png`(21종) + 생성된 `lib/constants/splash.ts`. 홈 헤더·로그인 배지는 `/app-icon-192.png`를 공유(교체 시 두 화면 자동 반영). OG 공유 이미지는 `app/opengraph-image.png`·`app/twitter-image.png`(1200×630, Next 파일 규칙으로 `og:image`/`twitter:image` 자동 주입), 절대 URL은 `layout.tsx`의 `metadataBase`(`https://foot-mate.vercel.app`)로 프로덕션 도메인 고정. ⚠️ 모든 PNG는 `sharp`로 SVG→래스터한 **산출물**이다. **로고의 단일 소스는 `scripts/gen-icons.mjs`·`scripts/gen-og.mjs`·`scripts/gen-splash.mjs`**(벡터가 스크립트에 인라인). 로고 수정은 스크립트의 `motif`/팔레트만 고쳐 `pnpm gen:assets`(또는 `gen:icons`/`gen:og`/`gen:splash`)로 전 사이즈 재생성 — **PNG를 직접 편집하지 말 것**. 세 스크립트의 `motif`는 동일하게 유지(로고 일관성). iOS 스플래시는 아래 TODO 섹션의 "스플래시 스크린" 항목 참고. deps: `sharp`·`png-to-ico`(devDependencies).
...
pnpm gen:assets                   # 브랜드 에셋(아이콘·OG·iOS 스플래시) 재생성 — 로고 수정 후 (scripts/gen-*.mjs, PNG 직접 편집 금지)
pnpm gen:splash                   # iOS 스플래시만 재생성 (public/splash/*.png + lib/constants/splash.ts) — 신기종 추가 시
```
...
- 기능 페이지: 커뮤니티는 **텍스트 게시판(공지·자유) + 댓글 + 이미지 첨부까지 구현 완료**. 사진은 별도 '갤러리 게시판'이 아니라 **어떤 글에나 붙는 선택 첨부**다(공지·자유 공통, 0장 허용) — `post_category` enum 의 `gallery` 값은 미사용으로 남겨둠(enum 값 삭제는 고통스럽고 무해). 데이터 모델은 별도 `post_images` 정션 테이블 대신 **`posts.images text[]` 컬럼**(경로 배열) — FK 2개짜리 정션 추가로 인한 PostgREST 임베드 모호성(댓글 목록이 조용히 사라졌던 그 함정)을 원천 회피. 이미지는 **`post-images` Storage 버킷**(공개 읽기 + 본인 uid 폴더 쓰기, `*_post_images.sql`, avatars와 동일 규칙)에 올리고 **경로만** 저장, 공개 URL은 `postImageUrl`(`lib/constants/community.ts`)로 렌더 시 조립. 작성 폼(`post-form.tsx`)은 카테고리 무관 다중 이미지 업로더(최대 `MAX_POST_IMAGES`=10, 5MB, RHF 가 images 소유)를 상시 노출. 목록은 썸네일(+N 배지)·상세는 이미지 그리드, 목록 필터에 **"사진" 탭**(`PHOTOS_FILTER`, `images <> '{}'` = `.neq("images", [])`, 카테고리 아님)으로 사진 있는 글만 모아보기. 삭제/수정 시 참조 끊긴 이미지는 `actions.ts`가 admin 클라이언트로 best-effort 정리(고아 파일 방지). 마이그레이션 `*_post_images.sql`(`posts.images` 컬럼 + `post-images` 버킷)은 프로덕션 적용 완료. (※ Storage는 아바타(`avatars`)·게시글 사진(`post-images`)·경기 사진(`match-images`) 세 버킷 배선됨.) ※ 로그인·클럽 생성/목록/상세/가입, **매치(일정·참석 투표·팀 편성·결과·쿼터 기록·사진·영상)**, 회비/정산, 알림, **커뮤니티(게시판·댓글·@멘션·이미지 첨부)** 는 구현 완료
  - 커뮤니티 라우트: `/clubs/[id]/community`(목록, 카테고리 필터 + "사진" 필터=이미지 보유 글) · `/new`(작성, 공지는 회장·총무·감독·코치만) · `/[postId]`(상세+댓글) · `/[postId]/edit`(수정, 작성자 본인 또는 회장·총무). 조회·댓글은 **활성 회원 전원(게스트 포함, RLS `is_club_member`)**. **글 작성은 정회원만(게스트 제외)** — RLS `post write`=`is_full_member`(`*_guest_post_write_fullmember.sql`), UI도 목록 글쓰기 버튼·`/new` 접근을 게스트에게 차단. 공지 작성만 운영진(RLS `post write`의 notice 분기). 스키마는 `lib/schemas/post.ts`, 카테고리 라벨·메타는 `lib/constants/community.ts`. 공지 등록→회원 전원 / 새 댓글→글 작성자 알림은 트리거(`*_community_notify.sql`의 `on_post_created`/`on_comment_created`, `notify_users` 경유). 매치·공지 생성 알림은 게스트 포함(활성 회원 전원, `*_match_notify_include_guests.sql`).
...
- 매치 라우트: `/clubs/[id]/matches`(목록) · `/new`(생성, 회장·총무·감독·코치) · `/[matchId]`(상세+참석투표+팀 편성+결과·기록+**사진·영상**) · `/[matchId]/edit`(수정). 날짜는 `lib/date.ts`(KST 고정), 라벨은 `lib/constants/matches.ts`, 스키마는 `lib/schemas/match.ts`. 참석 투표는 `vote_attendance` RPC(정원·대기자 자동 처리) 경유 — 수동 '모집 마감'(status)에 더해 매치에 `vote_deadline`(timestamptz, null이면 자동 마감 없음, `*_match_vote_deadline.sql`)이 설정돼 있으면 `now() > vote_deadline`에서 투표를 DB가 차단. 팀 편성·결과 모델은 위 "매치 도메인" 섹션 참고.
  - **경기 사진·영상**(`*_match_media.sql` + `*_match_videos_jsonb.sql`): 사진은 게시글과 같은 모델 — 정션 테이블 없이 **`matches.images text[]`**(경로 배열) + **`match-images` Storage 버킷**(공개 읽기 + 본인 uid 폴더 쓰기, post-images/avatars와 동일 규칙). 공개 URL은 `matchImageUrl`(`lib/constants/matches.ts`)로 조립, 최대 `MAX_MATCH_IMAGES`=20. 영상은 자체 저장(스토리지·대역폭 비용) 대신 외부 링크(유튜브 등)만 저장하되, 팀마다 방식이 달라(**풀 영상 1개 vs 쿼터별 여러 개**) 단일 컬럼이 아니라 **`matches.videos jsonb`(= `{label,url}` 배열)** 로 담는다. 저장 전 `parseMatchVideos`로 정규화, `youtubeEmbedUrl`로 임베드(유튜브 아니면 링크 칩), 최대 `MAX_MATCH_VIDEOS`=12, 라벨은 자유 입력 + `MATCH_VIDEO_LABEL_PRESETS`(전체·전반·후반·1~4쿼터) datalist 추천. 조회는 영상 2개+면 라벨 칩으로 하나 골라 재생(`match-videos.tsx`, 여러 iframe 쌓임 방지). **사진·영상은 매치 생성/수정 폼이 아니라 상세 페이지의 운영진 전용 관리 섹션**(`media-manager.tsx` → `saveMatchMedia` 액션, 스키마 `matchMediaSchema`)에서 붙인다(경기가 끝난 뒤 올리는 게 자연스러워서). 쓰기는 RLS `matches` UPDATE(=`can_manage_match` 운영진)가, 조회는 SELECT(=`is_club_member`, 게스트 포함 활성 회원 전원)가 지배. 참조 끊긴 사진은 저장/삭제 시 `actions.ts`가 admin 클라이언트로 best-effort 정리. 마이그레이션은 2개로 나뉜다 — `*_match_media.sql`(`images text[]` + `match-images` 버킷, **최초엔 `video_url text`로 만들었음**)과 `*_match_videos_jsonb.sql`(영상 모델을 `video_url`→`videos jsonb`로 전환: 컬럼 추가→기존 값 이관→`video_url` drop). 둘로 나눈 이유는 위 "데이터베이스"의 **적용된 마이그레이션 in-place 수정 금지** 함정 참고. 스키마 변경 후 `gen types` 필수.
...
- ~~스플래시 스크린~~ → 완료(모바일 한정). **Android**는 manifest(`background_color` + 512 아이콘)로 Chrome이 자동 합성 — 배선 불필요. **iOS**는 manifest를 안 읽으므로 `apple-touch-startup-image` link를 깐다: `scripts/gen-splash.mjs`(`pnpm gen:splash`, `gen:assets`에 포함)가 `public/splash/*.png` **21종(세로 전용)** + **`lib/constants/splash.ts`**(link 배열)를 생성하고, `layout.tsx`의 `metadata.appleWebApp.startupImage`가 이를 import한다. ⚠️ iOS는 `media` 쿼리(`device-width`/`device-height`/`-webkit-device-pixel-ratio`/`orientation`)가 기기와 **정확히 매칭될 때만** 표시하고 아니면 **흰 화면 폴백**이다 → 신기종이 나오면 스크립트의 `DEVICES`에 뷰포트(pt)·DPR을 추가하고 재실행할 것(현재 iPhone 8~17 Pro Max·Air·17e + iPad mini~Pro 13 M4 커버). 가로는 미커버(가로로 홈 화면 실행 시 흰 화면) — 세로 전용 앱이라 의도적. ⚠️ **확인은 홈 화면 추가 → 아이콘 실행(standalone)으로만 가능**하다 — Safari 탭에서는 스플래시가 아예 뜨지 않으므로 "안 보인다"는 대개 배선 문제가 아니라 실행 방식 문제다. 안 뜰 때 점검 순서: (1) **manifest가 200인지**(위 `proxy.ts` matcher 함정 — standalone이 깨지면 스플래시도 통째로 안 나온다), (2) 홈 화면 아이콘을 **삭제 후 재추가**했는지(iOS는 추가 시점의 link를 캡처해 보관, 서버가 바뀌어도 갱신 안 됨), (3) 기기 뷰포트가 `DEVICES`에 있는지, (4) Safari 탭이 아니라 아이콘으로 콜드 런치했는지. Next 16이 `appleWebApp.capable`을 표준 이름(`mobile-web-app-capable`)으로만 내보내므로 구형 iOS(16.4 미만) 대비로 `metadata.other`에 `apple-mobile-web-app-capable: yes`를 명시해 둔다. **이미지 레이아웃은 Android(Chrome) 합성 스플래시에 맞춘 것**(두 플랫폼 통일): 평면 `#f6f8f4` 배경 + 화면 중앙 로고 배지(폭의 24%, 최대 132pt) + 그 아래 "Foot Mate" 한 줄. ⚠️ **Android는 반대로 맞출 수 없다** — manifest에 스플래시 이미지 필드가 없고 Chrome이 `background_color`+512 아이콘+`name`을 고정 레이아웃으로 합성하는 게 전부라, 통일하려면 iOS를 그쪽에 맞추는 방향뿐이다. 그래서 부제·하단 라인·배경 오브·그리드·그림자를 **의도적으로 뺐다**(넣으면 Android와 갈린다). 덕분에 팔레트 256 + `dither:0` 인코딩으로 총 ~264KB, 기기당 1장(iPhone ~11KB)뿐이다. **PC(데스크톱 PWA)는 스플래시가 없다** — Chrome·Edge는 설치 PWA에도 스플래시를 띄우지 않고(창을 `background_color`로 잠깐 칠하는 게 전부) 이를 바꿀 표준 API가 없다. 앱 내 오버레이로 흉내낼 순 있으나 `loading.tsx` 스켈레톤과 중복 + LCP 손해라 **의도적으로 넣지 않음**.
...
- ~~Web Push 발송 Edge Function~~ → 완료. **Edge Function이 아니라 Next.js Node Route Handler로 구현**: `notifications` INSERT → (Supabase Database Webhook) → `POST /api/push/send`(`app/api/push/send/route.ts`, `runtime=nodejs`, `PUSH_WEBHOOK_SECRET`로 보호) → `lib/push/send.ts`(`web-push` + VAPID, admin 클라이언트로 `push_subscriptions` 조회·병렬 발송, 404/410 만료 구독 자동 삭제) → `public/sw.js`의 `push` 이벤트 → `showNotification`. 구독 저장/해제는 `app/me/push-actions.ts`·`components/push/use-push.ts`. VAPID/웹훅 시크릿 env는 `.env.example` 참고. 발송 트리거(notifications INSERT → `/api/push/send`)는 **`*_push_webhook.sql` 마이그레이션**으로 배선됨(`pg_net` + `notify_push_webhook` 트리거). URL·시크릿은 **Vault**에서 읽는다(`push_webhook_url`/`push_webhook_secret`, `vault.create_secret`로 환경별 1회 주입 — git 커밋 금지). Vault 미주입 시 발송만 스킵(알림 생성은 정상). ⚠️ 신규/스테이징 환경 세팅 시 **기존 대시보드 Database Webhook을 반드시 비활성화**(이중 발송 방지). pg_net·Vault 모두 fire-and-forget이라 재시도·관측은 없음. **프로덕션(`foot-mate.vercel.app`)엔 이미 마이그레이션 적용 + Vault 주입 + 대시보드 웹훅 제거 + 스모크테스트 통과 완료** — 위 경고는 새 환경 세팅용.
- ~~구장 위치 지도: 카카오맵 SDK~~ → 완료. 매치의 `location_lat`/`location_lng`(이미 있던 컬럼) 활용 — 마이그레이션 없음. SDK는 npm이 아니라 **스크립트 로드**(`//dapi.kakao.com/v2/maps/sdk.js?appkey=...&autoload=false&libraries=services`), 로더 훅 `components/map/use-kakao-loader.ts`가 페이지당 한 번만 삽입(모듈 레벨 프로미스 dedup) + `kakao.maps.load()` 초기화, 키(`NEXT_PUBLIC_KAKAO_MAP_KEY`) 미설정/실패 시 error 상태로 폴백. 타입은 자체 선언 `types/kakao-maps.d.ts`(공식 @types 없음, 쓰는 표면만). **생성/수정 폼**은 `location-picker.tsx`(장소 이름 자유 입력 + 카카오 장소 검색 → 결과 선택 시 이름·좌표 동시 설정, 좌표 있으면 지도 미리보기 + 드래그로 미세조정, "위치 지우기"로 좌표만 제거) — 폼 값 계약은 문자열 3개(`locationName`/`locationLat`/`locationLng`, `matchSchema`가 `optionalCoord`로 coerce + 위/경도 동반 존재 refine). **상세**는 좌표 있으면 읽기 전용 `components/map/match-map.tsx` + 카카오맵 외부 링크(`map.kakao.com/link/map|to/이름,lat,lng`, SDK 없이도 동작). ⚠️ 키는 카카오 OAuth와 **같은 앱의 JavaScript 키**, 콘솔 > 플랫폼 > Web 에 사용 도메인(`localhost:4000`·프로덕션) 등록 필요(도메인 화이트리스트라 secret 아님 → `NEXT_PUBLIC_` OK). ⚠️ **키·도메인만으론 부족 — 콘솔 > 제품 설정 > 카카오맵(지도/로컬)을 활성화**해야 한다. 안 켜면 `sdk.js`가 **403 `{"errorType":"NotAuthorizedError","message":"App(...) disabled OPEN_MAP_AND_LOCAL service."}`** 로 막혀 `<script onerror>` → 로더가 `load-failed` → "지도 검색을 사용할 수 없어..." 폴백만 뜬다(키가 정상이어도 동일 증상). 지도 타일과 장소 검색(Local API, `libraries=services`)이 이 활성화 하나로 함께 열린다. 빠른 진단: `curl "https://dapi.kakao.com/v2/maps/sdk.js?appkey=<KEY>&autoload=false&libraries=services"` → 403이면 제품 미활성, 200이면 정상.
  - ⚠️ **RHF: 크로스-필드 refine이 걸린 여러 필드를 한 이벤트에서 갱신할 땐 필드별 `shouldValidate` 금지 — 마지막에 `trigger([...])` 한 번**. `location-picker` 결과 선택 시 name/lat/lng 3개를 각각 `setValue(..., {shouldValidate:true})` 하면, 중간 단계(lat만 채워진 반쪽 상태)에서 refine이 `locationLat` 슬롯에 에러를 심고, 뒤 필드의 `trigger(name)`은 **resolver로 전체를 돌려도 그 name의 에러 슬롯만 갱신**하므로 앞서 심긴 stale 에러를 못 지운다 → 값은 정상 저장되는데 "위치 좌표가 올바르지 않아요"가 화면에 남는다. 해소: 세 값을 먼저 다 `setValue`(검증 없이)한 뒤 `form.trigger(["locationName","locationLat","locationLng"])`로 완성 상태에서 한 번에 검증. 멀티필드 피커(좌표·범위 등)를 새로 만들면 이 패턴을 따를 것.
