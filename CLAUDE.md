...
- DB 타입: `supabase gen types typescript --project-id ezvurwzfvcgxkhbntqxb > lib/supabase/database.types.ts`로 생성, 4개 클라이언트(client/server/admin/middleware)에 `<Database>` 제네릭 배선 완료 → 쿼리·insert·update가 타입 안전. **스키마 변경 시 이 파일을 재생성**할 것. ⚠️ **읽기 경로의 `as unknown as` 캐스팅(임베드 쿼리 결과 → 손수 정의한 Row 타입)은 아직 남아있음**(13개 파일 ~27곳) — 제네릭 클라이언트라 이제 결과에 실타입이 붙지만, 임베드(`profiles!author_id(...)`) 결과 타입이 손수 타입과 정확히 안 맞아 점진적 제거가 필요. 신규 코드는 캐스팅 대신 생성 타입(`Database["public"]["Tables"][...]["Row"]`)을 우선.
...
- ~~Web Push 발송 Edge Function~~ → 완료. **Edge Function이 아니라 Next.js Node Route Handler로 구현**: `notifications` INSERT → (Supabase Database Webhook) → `POST /api/push/send`(`app/api/push/send/route.ts`, `runtime=nodejs`, `PUSH_WEBHOOK_SECRET`로 보호) → `lib/push/send.ts`(`web-push` + VAPID, admin 클라이언트로 `push_subscriptions` 조회·병렬 발송, 404/410 만료 구독 자동 삭제) → `public/sw.js`의 `push` 이벤트 → `showNotification`. 구독 저장/해제는 `app/me/push-actions.ts`·`components/push/use-push.ts`. VAPID/웹훅 시크릿 env는 `.env.example` 참고. ⚠️ **Database Webhook(notifications INSERT → `/api/push/send`)은 마이그레이션이 아니라 Supabase 대시보드 수동 설정 전제** — 프로젝트 이전/재생성 시 재등록 필요. (백업용 **이메일 알림은 아직 미구현**)
