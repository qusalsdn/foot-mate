...
**서버 컴포넌트 쿼리는 병렬로**: 페이지 로딩 시간 = 서버가 도는 모든 조회의 합이고, 무료 티어·콜드 스타트 지연이 순차 `await`마다 곱해진다. 서로 **독립인** Supabase 조회는 `Promise.all`로 묶는다. 의존 쿼리(A 결과로 B를 필터)만 순차 유지하고, **가드(`notFound`/`redirect`)는 배치를 다 받은 뒤 원래 순서대로** 검사한다(병렬화해도 최종 404/리다이렉트 결과는 동일). 조건부 쿼리는 권한 없으면 `null` 브랜치로 배열에 태워 스킵한다(예: 클럽 상세의 로스터=정회원만·가입대기=회장/총무만 → `isFullMember ? supabase... : null`, 결과는 `res?.data ?? []`). **적용 현황: 모든 서버 컴포넌트 페이지(14개)에 반영됨** — 목록/상세 9개(홈 6→3, 클럽 상세 4→2, 매치 목록/상세, 커뮤니티 목록/상세, 회비, 알림, 회원 프로필 6→2, 게시글 상세 5→2) + 폼 5개(매치·커뮤니티 각 new/edit). 무거웠던 회원 프로필·게시글 상세가 대표적. **신규 서버 컴포넌트 페이지도 이 패턴을 따르고, 함께 `loading.tsx`(스켈레톤)를 둘 것**(아래 "디자인/UI 컨벤션 > 로딩 스켈레톤" 참고).
